...
export default function Page({ rows, onDataChange }: TableProps) {
    ...
    const [editItem, setEditItem] = React.useState<PaymentMethod | null>(null);

    React.useEffect(() => {
        setRows(rows);
    }, [rows]);

    const updateRows = (newRows: PaymentMethod[]) => {
        setRows(newRows);
    ...
    const handleSaveClick = (data: PaymentMethod) => {
        const updated = rowsData?.find(x => x.payment_method_id === data.payment_method_id)
            ? rowsData.map(x => (x.payment_method_id === data.payment_method_id ? data : x))
            : [...rowsData, data];
    ...
    const handleEditClick = (item: PaymentMethod) => {
        setEditItem(item);
        setView('edit');
    };
    ...
    const handleDeleteReq = (data: PaymentMethod) => {
        const updated = rowsData.filter(row => row.payment_method_id !== data.payment_method_id);
        updateRows(updated);
            ...
            <Table sx={{ tableLayout: 'fixed' }} aria-label="custom pagination table">
                ...
                <TableBody>
                    ...
                    {(rowsPerPage > 0 && rowsData
                        ...
                        view === 'edit' && editItem?.payment_method_id === row?.payment_method_id ? (
                            <EditPanel
                                row={row}
                                onBack={handleBackClick}
                                onSave={handleSaveClick}
                            />
                        ) : view === 'delete' && editItem?.payment_method_id === row?.payment_method_id ? (
                            <DeletePanel
                                row={row}
                                onBack={handleBackClick}

...
import api, { convertDateTimeMin, formatNumber, getCurrentTimeMin } from '@/utils/http_helper';
...
/** NEW: draft type for Stock edit mode */
type StockDraft = {
  inventory_id: number;
  /** new stock value the user typed (number) */
  stock: string;
  /** date/time string to send (from props.time) */
  date: string;
};
type StockDraftMap = Record<number, StockDraft>;
...
export default function Page({ time, rows, info, error, setIsShow, filterSupplierId, setFilterSupplierId }: TableProps) {
  ...
  const [view, setView] = React.useState<'hide' | 'add' | 'edit' | 'delete' | 'multi' | 'stock' | 'modify'>('hide');
  ...
  const clearDebounce = (id: number) => {
    const t = debouncersRef.current[id];
    if (t) {
      clearTimeout(t);
      debouncersRef.current[id] = null;
    }
  };

  const postUpdateValue = (inventory_id: number, valueStr: string | null | undefined) => {
    // treat '' as null, but keep '0' as 0
    const trimmed = valueStr?.trim();
    const num = (trimmed === '' || trimmed == null) ? null : Number(trimmed);
    // NOTE: allow 0; only null if truly empty/invalid
    const payload = { inventory_id, value: (num === null || Number.isNaN(num)) ? null : num };
    return api.post('/update-inventory-check-value', payload)
      .catch(err => console.error(t('unexpected_error'), err));
  };
  ...
  /** NEW: state to hold per-row stock edits in Stock mode */
  const [stockDrafts, setStockDrafts] = React.useState<StockDraftMap>({});

  /** helper to update a single row's stock draft */
  const updateStockDraft = (id: number, patch: Partial<StockDraft>) => {
    setStockDrafts(prev => ({
      ...prev,
      [id]: {
        inventory_id: id,
        stock: prev[id]?.stock ?? '',
        date: prev[id]?.date ?? time,
        ...patch,
      },
    }));
  };
  ...
  React.useEffect(() => {
    ...
    // keep stockDrafts aligned to rows too
    setStockDrafts(prev => {
      const next: StockDraftMap = { ...prev };
      // add missing entries, default stock to current_stock
      rows.forEach(r => {
        if (!next[r.inventory_id]) {
          next[r.inventory_id] = {
            inventory_id: r.inventory_id,
            stock: String(Number(r?.current_stock ?? 0)),
            date: time,
          };
        }
      });
      // remove drafts for rows that no longer exist
      Object.keys(next).forEach(k => {
        if (!rows.some(r => r.inventory_id === Number(k))) {
  ...
  const handleModifyClick = () => {
    setStockDrafts(() => {
      ...
      rowsData?.forEach(r => {
        m[r.inventory_id] = {
          inventory_id: r.inventory_id,
          stock: (r?.current_stock == null) ? '' : String(Number(r.current_stock)),
          date: time,
        };
      });
      return m;
  ...
  const handleStockChange = async (isChange = false) => {
    const payload = Object.values(stockDrafts)
      .filter(d => d.stock.trim() !== '' && Number.isFinite(Number(d.stock)))
      .map(d => ({
        inventory_id: d.inventory_id,
        stock: Number(d.stock),
      }));

    const param = isChange ? { date: time, data: payload } : { data: payload };
    ...
    try {
      ...
      info(t('update_success'));
      error('');
      setRows(prev => prev.map(r => {
        const changed = payload.find(p => p.inventory_id === r.inventory_id);
        return changed ? { ...r, current_stock: changed.stock } : r;
      }));
      ...
      <Table sx={{ tableLayout: 'fixed' }} stickyHeader aria-label="sticky table">
          ...
          <TableRow>
            ...
            {view === 'multi' ? (
              ...
              <StyledTableCell sx={{ width: 280 }}>
                ...
                <Tooltip title={t('multiple_stock_check')}>
                  <IconButton
                    aria-label="multiple_stock_check"
                    color="warning"
                ...
                <Tooltip title={t('modify_multiple_stock')}>
                  <IconButton
                    aria-label="modify_multiple_stock"
                    color="error"
        ...
        <TableBody>
          ...
          {pagedRows?.map((row) => {
            ...
            // value shown in Stock mode input (falls back to row.current_stock)
            const stockDraftVal =
              stockDrafts[row.inventory_id]?.stock ??
              (row?.current_stock == null ? '' : String(Number(row.current_stock)));
            ...
            return (
              view === 'edit' && editItem?.inventory_id === row?.inventory_id ? (
                ...
                <TableRow key={row?.inventory_id}>
                  ...
                  {view === 'stock' ? (
                    <TableCell style={{ padding: 8 }}>
                      <TextField
                        ...
                        label={t('quantity')}
                        onFocus={() => {
                          setFocusedInventoryId(row.inventory_id);
                          // post current value on focus (may be prefilled by polling)
                          const current = stockDrafts[row.inventory_id]?.stock ?? '';
                          postUpdateValue(row.inventory_id, current);
                        }}

                        onChange={(e) => {
                          const val = e.target.value;
                          // keep your local draft in sync
                          updateStockDraft(row.inventory_id, { stock: val, date: time });

                          // debounce POST while typing
                          clearDebounce(row.inventory_id);
                          debouncersRef.current[row.inventory_id] = setTimeout(() => {
                            postUpdateValue(row.inventory_id, val);
                          }, 50);
                        }}

                        onBlur={() => {
                          // flush last value immediately on blur
                          clearDebounce(row.inventory_id);
                          const finalVal = stockDrafts[row.inventory_id]?.stock ?? '';
                          postUpdateValue(row.inventory_id, finalVal);
                          setFocusedInventoryId(null);
                        }}
                    ...
                    <TableCell style={{ padding: 8 }}>
                      <TextField
                        ...
                        type="number"
                        onChange={(e) => {
                          updateStockDraft(row.inventory_id, {
                  ...
                  ) : (<TableCell>{formatNumber(row?.current_stock) || '--'}</TableCell>)}
                  <TableCell>

...
import api, { convertDateTime, convertDateTimeMin, getCurrentTimeMin } from '@/utils/http_helper';
...
export default function Page({ rows }: TableProps) {
    ...
    const handlePurchaseClick = (item: Inventory) => {
        setEditItem(item);
        setView('purchase');
    };
            ...
            <Table sx={{ tableLayout: 'fixed' }} stickyHeader aria-label="sticky table">
                <TableHead>
                    <TableRow>
                        ...
                        <StyledTableCell sx={{ width: 160 }}>{t('stock')}</StyledTableCell>

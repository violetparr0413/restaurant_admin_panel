...
export default function Page({ rows, onDataChange }: TableProps) {
    ...
    const updateRows = (newRows: Service[]) => {
        setRows(newRows);
    ...
    const handleSaveClick = (data: Service) => {
        const updated = rowsData?.find(x => x.service_id === data.service_id)
            ? rowsData.map(x => (x.service_id === data.service_id ? data : x))
            : [...rowsData, data];
    ...
    const handleDeleteReq = (data: Service) => {
        const updated = rowsData.filter(row => row.service_id !== data.service_id);
        updateRows(updated);
